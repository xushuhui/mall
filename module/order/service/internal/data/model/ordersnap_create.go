...
func (osc *OrderSnapCreate) check() error {
	if _, ok := osc.mutation.SnapImg(); !ok {
		return &ValidationError{Name: "snap_img", err: errors.New(`model: missing required field "OrderSnap.snap_img"`)}
	}
	if _, ok := osc.mutation.SnapTitle(); !ok {
		return &ValidationError{Name: "snap_title", err: errors.New(`model: missing required field "OrderSnap.snap_title"`)}
	}
	if _, ok := osc.mutation.SnapItems(); !ok {
		return &ValidationError{Name: "snap_items", err: errors.New(`model: missing required field "OrderSnap.snap_items"`)}
	}
	if _, ok := osc.mutation.SnapAddress(); !ok {
		return &ValidationError{Name: "snap_address", err: errors.New(`model: missing required field "OrderSnap.snap_address"`)}
	}
	return nil
}
