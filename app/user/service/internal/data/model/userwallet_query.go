...
// Last returns the last UserWallet entity from the query.
// Returns a *NotFoundError when no UserWallet was found.
func (uwq *UserWalletQuery) Last(ctx context.Context) (*UserWallet, error) {
...
func (uwq *UserWalletQuery) LastX(ctx context.Context) *UserWallet {
	node, err := uwq.Last(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
...
// LastID returns the last UserWallet ID from the query.
// Returns a *NotFoundError when no UserWallet ID was found.
func (uwq *UserWalletQuery) LastID(ctx context.Context) (id int64, err error) {
	var ids []int64
	if ids, err = uwq.IDs(ctx); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{userwallet.Label}
...
func (uwq *UserWalletQuery) LastIDX(ctx context.Context) int64 {
	id, err := uwq.LastID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
...
// Returns a *NotSingularError when exactly one UserWallet entity is not found.
...
// Returns a *NotSingularError when exactly one UserWallet ID is not found.
