...
func (uwc *UserWalletCreate) check() error {
	if _, ok := uwc.mutation.CreateTime(); !ok {
		return &ValidationError{Name: "create_time", err: errors.New(`model: missing required field "UserWallet.create_time"`)}
	}
	if _, ok := uwc.mutation.UpdateTime(); !ok {
		return &ValidationError{Name: "update_time", err: errors.New(`model: missing required field "UserWallet.update_time"`)}
	}
	if _, ok := uwc.mutation.Value(); !ok {
		return &ValidationError{Name: "value", err: errors.New(`model: missing required field "UserWallet.value"`)}
	}
	return nil
}
