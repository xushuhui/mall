...
func initApp(confServer *conf.Server, confData *conf.Data, registry *conf.Registry, logger log.Logger) (*kratos.App, func(), error) {
	...
	tagRepo := data.NewTagRepoRepo(dataData, logger)
	tagUsecase := biz.NewTagUsecase(tagRepo, logger)
	appService := service.NewAppService(bannerUsecase, themeUsecase, activityUsecase, cateGoryUsecase, tagUsecase, logger)
