...
// Last returns the last UserPoint entity from the query.
// Returns a *NotFoundError when no UserPoint was found.
func (upq *UserPointQuery) Last(ctx context.Context) (*UserPoint, error) {
...
func (upq *UserPointQuery) LastX(ctx context.Context) *UserPoint {
	node, err := upq.Last(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
...
// LastID returns the last UserPoint ID from the query.
// Returns a *NotFoundError when no UserPoint ID was found.
func (upq *UserPointQuery) LastID(ctx context.Context) (id int64, err error) {
	var ids []int64
	if ids, err = upq.IDs(ctx); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{userpoint.Label}
...
func (upq *UserPointQuery) LastIDX(ctx context.Context) int64 {
	id, err := upq.LastID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
...
// Returns a *NotSingularError when exactly one UserPoint entity is not found.
...
// Returns a *NotSingularError when exactly one UserPoint ID is not found.
