// Code generated by Wire. DO NOT EDIT.

//go:generate go run github.com/google/wire/cmd/wire
//go:build !wireinject
// +build !wireinject

package main

import (
	"github.com/go-kratos/kratos/v2"
	"github.com/go-kratos/kratos/v2/log"
	"mall-go/app/mall/interface/internal/biz"
	"mall-go/app/mall/interface/internal/conf"
	"mall-go/app/mall/interface/internal/data"
	"mall-go/app/mall/interface/internal/server"
	"mall-go/app/mall/interface/internal/service"
)

// Injectors from wire.go:

// initApp init kratos application.
func initApp(confServer *conf.Server, confData *conf.Data, registry *conf.Registry, logger log.Logger) (*kratos.App, func(), error) {
	discovery := data.NewDiscovery(registry)
	interfaceClient := data.NewAppServiceClient(discovery)
	dataData, cleanup, err := data.NewData(interfaceClient, logger)
	if err != nil {
		return nil, nil, err
	}
	bannerRepo := data.NewBannerRepo(dataData, logger)
	bannerUsecase := biz.NewBannerUsecase(bannerRepo, logger)
	serviceInterface := service.NewInterface(bannerUsecase, logger)
	httpServer := server.NewHTTPServer(confServer, serviceInterface, logger)
	grpcServer := server.NewGRPCServer(confServer, serviceInterface, logger)
	registrar := data.NewRegistrar(registry)
	app := newApp(logger, httpServer, grpcServer, registrar)
	return app, func() {
		cleanup()
	}, nil
}
