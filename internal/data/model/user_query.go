...
func (uq *UserQuery) QueryRefund() *RefundQuery {
	query := &RefundQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// Last returns the last User entity from the query.
// Returns a *NotFoundError when no User was found.
func (uq *UserQuery) Last(ctx context.Context) (*User, error) {
...
func (uq *UserQuery) LastX(ctx context.Context) *User {
	node, err := uq.Last(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
...
// LastID returns the last User ID from the query.
// Returns a *NotFoundError when no User ID was found.
func (uq *UserQuery) LastID(ctx context.Context) (id int64, err error) {
	var ids []int64
	if ids, err = uq.IDs(ctx); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{user.Label}
...
func (uq *UserQuery) LastIDX(ctx context.Context) int64 {
	id, err := uq.LastID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withRefund: uq.withRefund.Clone(),
...
func (uq *UserQuery) WithRefund(opts ...func(*RefundQuery)) *UserQuery {
	query := &RefundQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withRefund; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int64]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Refund = []*Refund{}
		}
		query.Where(predicate.Refund(func(s *sql.Selector) {
			s.Where(sql.InValues(user.RefundColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			...
			node.Edges.Refund = append(node.Edges.Refund, n)
		}
	}
