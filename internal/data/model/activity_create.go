...
func (ac *ActivityCreate) AddSpuIDs(ids ...int64) *ActivityCreate {
	ac.mutation.AddSpuIDs(ids...)
	return ac
}

// AddSpu adds the "spu" edges to the Spu entity.
func (ac *ActivityCreate) AddSpu(s ...*Spu) *ActivityCreate {
	ids := make([]int64, len(s))
	for i := range s {
		ids[i] = s[i].ID
	}
	return ac.AddSpuIDs(ids...)
}

// AddCouponIDs adds the "coupon" edge to the Coupon entity by IDs.
func (ac *ActivityCreate) AddCouponIDs(ids ...int64) *ActivityCreate {
	ac.mutation.AddCouponIDs(ids...)
	return ac
}

// AddCoupon adds the "coupon" edges to the Coupon entity.
func (ac *ActivityCreate) AddCoupon(c ...*Coupon) *ActivityCreate {
	ids := make([]int64, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return ac.AddCouponIDs(ids...)
}
...
func (ac *ActivityCreate) createSpec() (*Activity, *sqlgraph.CreateSpec) {
	...
	if nodes := ac.mutation.SpuIDs(); len(nodes) > 0 {
	...
	if nodes := ac.mutation.CouponIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
